...
if __name__ == "__main__":
    ...
    y = []
    t = []
    for percentage in range(10,11):
        start = time.time()
        mp.compute_statistics(ALL_TRAINING_IMAGES, ALL_TRAINING_LABELS, DATA_WIDTH,
            DATA_HEIGHT, mp.extract_final_features, percentage*10)
        ...
        for ei in range(len(predicted_labels)):
            if(ALL_VALIDATION_LABELS[ei] == predicted_labels[ei]):
                ...
                print
                print "Predicted:", predicted_labels[ei], "Actual:",ALL_VALIDATION_LABELS[ei]
                _print_digit_image(ALL_VALIDATION_IMAGES[ei])
                print

        print "Percentage used:", percentage*10
        y.append(float(float(correct_count)/float(len(predicted_labels))))
        print "Correct prediction: " + str(float(float(correct_count)/float(len(predicted_labels))))
